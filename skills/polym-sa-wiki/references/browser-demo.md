...
- **Always remove the previous step's highlight before adding the new one** (track via a known CSS class, e.g. `.polym-demo-highlight`)

### 3.5 GIF recording

- Start `gif_creator` at step 1.
- Stop & export at the end of the demo (after the last step or on graceful abort).
- Default save path: `~/Desktop/polym-demo-<slug>-<YYYYMMDD-HHMM>.gif` where `<slug>` is a kebab-case form of the tutorial title.
...
This reference file intentionally lives under `polym-sa-wiki/references/` rather than a shared skill, because:
...
If a **future skill** (e.g. `polym-help` showing how to install Polym itself) wants the same browser-demo behaviour, it should `Read` this file from its own SKILL.md rather than re-implement. Treat this file as the team's shared demo runbook — but only invoked, never triggered.
