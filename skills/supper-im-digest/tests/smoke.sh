#!/usr/bin/env bash
# Smoke test for supper-im-digest skill
set -euo pipefail

SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "supper-im-digest smoke: checking lark-cli..."
command -v lark-cli >/dev/null || { echo "FAIL: lark-cli not found"; exit 1; }
echo "  OK: lark-cli found"

echo "supper-im-digest smoke: checking reference files..."
...
echo "supper-im-digest smoke: verifying lark-cli im commands available..."
...
echo "supper-im-digest smoke: PASS"
