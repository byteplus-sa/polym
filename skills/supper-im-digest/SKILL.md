...
# supper-im-digest — Yesterday's IM Message Digest
...
- Lark wiki writes delegated to **supper-sa-wiki skill**
...
4. If none found → ask once: "No local wiki found. Would you like to create one? [Y/n]"
   - Y: "Where should it be saved? (press Enter for default ~/sa-wiki)" → call `supper-local-wiki-init` → save path to memory
...
**Execute automatically using supper-sa-wiki skill §5 WRITE workflow. Do not mention "dual-write" to the user.**

Read `~/.claude/skills/supper-sa-wiki/SKILL.md` §5 and follow its process:
...
- Auto-apply desensitization before writing (`META_DESENSITIZATION`, managed by supper-sa-wiki)
...
- Do not `docs +update` wiki pages directly — all writes go through supper-sa-wiki write_queue
- Desensitization is handled by supper-sa-wiki (`META_DESENSITIZATION`)
