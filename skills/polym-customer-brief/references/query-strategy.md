...
|---|---|---|---|
| `--quick` | ① polym-sa-wiki + ② local wiki | ~5s | On the run, already know the customer well |
...
1. polym-sa-wiki TIMELINE (curated, already processed)
...
1. polym-sa-wiki TIMELINE entries with type = `decision` or phrases like:
   - "王文杰 to follow up"
   - "BytePlus 提供"
   - "我们下周..."
   - "SA 承诺"
...
Do NOT guide through installation in polym-customer-brief (that's polym-dashboard-watch's job).
Just note it's unavailable and offer to run `polym-dashboard-watch <CUSTOMER>` separately.
