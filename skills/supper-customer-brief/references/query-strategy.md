...
|---|---|---|---|
| `--quick` | ① supper-sa-wiki + ② local wiki | ~5s | On the run, already know the customer well |
...
1. supper-sa-wiki TIMELINE (curated, already processed)
...
1. supper-sa-wiki TIMELINE entries with type = `decision` or phrases like:
   - "王文杰 to follow up"
   - "BytePlus 提供"
   - "我们下周..."
   - "SA 承诺"
...
Do NOT guide through installation in supper-customer-brief (that's supper-dashboard-watch's job).
Just note it's unavailable and offer to run `supper-dashboard-watch <CUSTOMER>` separately.
