---
name: supper-sa-super-skill-help
version: 0.2.2
...
# supper-sa-super-skill-help — Onboarding Guide
...
─────────────────────────────────────────────────────────────────
Lark group chats      →  supper-im-digest          →  knowledge base
Live meetings         →  supper-meeting-recorder   →  knowledge base
Lark VC / Minutes     →  supper-meeting-summary    →  knowledge base
C360 usage data       →  supper-dashboard-watch    →  knowledge base (on demand)
Lark documents        →  supper-lark-doc-ingest    →  knowledge base

knowledge base
  └── supper-sa-wiki (Lark Bitable + Wiki)    ← shared SA team wiki
  └── local wiki (~/sa-wiki/)         ← your private local copy

Consumer skills
  supper-customer-brief   →  pulls everything about one customer before a meeting
```
...
|---|---|---|
| Morning: catch up on yesterday's chats | `supper-im-digest` | "Organize yesterday's messages" |
| During a meeting: record and transcribe live | `supper-meeting-recorder` | "Start meeting recording" |
| Morning: see what meetings happened | `supper-meeting-summary` | "Summarize yesterday's meetings" |
| Pre-meeting: prep for a customer | `supper-customer-brief` | "Give me a brief on Acme before my 2pm" |
| Ad-hoc: how much is X using? | `supper-dashboard-watch` | "How many tokens did Acme use last month?" |
| Ad-hoc: save a Lark doc to the wiki | `supper-lark-doc-ingest` | "Ingest this document" |
| First time: create your local wiki | `supper-local-wiki-init` | "Set up my local wiki" |
| Searching SA knowledge | `supper-sa-wiki` | "What do we know about AK/SK rotation?" |
...
This installs the core skills: `supper-sa-wiki`, `supper-meeting-recorder`, `supper-meeting-summary`, `supper-im-digest`, `supper-dashboard-watch`, `supper-local-wiki-init`, `supper-customer-brief`, and `supper-sa-super-skill-help`.
...
### Step 5: Chrome extension (for supper-dashboard-watch only)

`supper-dashboard-watch` queries C360 via browser automation. You need the **Claude for Chrome** extension:
...
### Step 5b: Syncore setup (for supper-meeting-recorder)

`supper-meeting-recorder` uses Syncore to capture mic + system audio.
...
```
"Organize yesterday's messages"          → supper-im-digest
"Start meeting recording"                → supper-meeting-recorder
"End meeting"                            → supper-meeting-recorder
"Summarize yesterday's meetings"         → supper-meeting-summary
"Brief me on Acme before my meeting"     → supper-customer-brief
"Acme used how many tokens last month?"  → supper-dashboard-watch
"Ingest this document: <URL>"            → supper-lark-doc-ingest
"What do we know about Seedance quotas?" → supper-sa-wiki
"Set up my local wiki"                   → supper-local-wiki-init
```

### Tips

- **First run**: skills ask for your local wiki path once, then remember it. You won't be asked again.
- **Knowledge accumulates**: the more you run `supper-im-digest`, `supper-meeting-recorder`, and `supper-meeting-summary`, the richer your wiki becomes, and the better `supper-customer-brief` gets.
- **No manual filing**: skills save useful outputs to the knowledge base by default. You only see the final report.
- **supper-customer-brief modes**: use `--quick` for a 5-second wiki-only brief, or `--full` for a 20-second brief that also pulls C360 data.
...
Look at `skills/supper-im-digest/SKILL.md` for a good example of structure.
...
# Develop
super-skill new <name>                  # scaffold a new skill (auto-prefixes to supper-<name>)
