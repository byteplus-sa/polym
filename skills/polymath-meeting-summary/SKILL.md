...
# polymath-meeting-summary — Meeting Content Compilation
...
- Lark wiki writes delegated to the **polymath-sa-wiki skill**
...
4. None found → ask once: "No local wiki found yet. Would you like to create one? [Y/n]"
   - Y: "Where should it be saved? (press Enter for default ~/sa-wiki)" → call `polymath-local-wiki-init` → save to memory
...
- Execute desensitization before writing (`META_DESENSITIZATION`, managed by polymath-sa-wiki)
...
- Desensitization is handled by polymath-sa-wiki
