...
# polym-meeting-summary — Meeting Content Compilation
...
- Lark wiki writes delegated to the **polym-sa-wiki skill**
...
4. None found → ask once: "No local wiki found yet. Would you like to create one? [Y/n]"
   - Y: "Where should it be saved? (press Enter for default ~/sa-wiki)" → call `polym-local-wiki-init` → save to memory
...
**Execute automatically using polym-sa-wiki skill §5 WRITE workflow.**
...
- Execute desensitization before writing (`META_DESENSITIZATION`, managed by polym-sa-wiki)
...
- Desensitization is handled by polym-sa-wiki
