...
# polymath-lark-doc-ingest — Lark Document Ingestion

Scan Lark documents within a time range, extract key content, and write to the knowledge base. Like `polymath-im-digest` / `polymath-meeting-summary`, saves automatically without interrupting the user.
...
```
polymath-lark-doc-ingest                          # Default: yesterday 00:00 to now
polymath-lark-doc-ingest --url <lark-doc-url>     # Specify a single document
polymath-lark-doc-ingest --days 3                 # Custom time range (days)
polymath-lark-doc-ingest --folder <folder-token>  # Restrict to a specific folder
polymath-lark-doc-ingest --dry-run                # Preview without writing
```
...
After extraction, run **desensitization check** (polymath-sa-wiki `META_DESENSITIZATION` rules):
...
**Execute automatically using polymath-sa-wiki skill §5 WRITE workflow.**
...
Run polymath-sa-wiki desensitization workflow before writing.
...
- polymath-sa-wiki SKILL.md §5 (WRITE workflow)
