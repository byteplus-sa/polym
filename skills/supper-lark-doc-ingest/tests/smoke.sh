#!/usr/bin/env bash
# Smoke test for supper-lark-doc-ingest skill
set -euo pipefail

echo "supper-lark-doc-ingest smoke: checking lark-cli..."
command -v lark-cli >/dev/null || { echo "FAIL: lark-cli not found"; exit 1; }

echo "supper-lark-doc-ingest smoke: checking drive and docs commands..."
lark-cli drive --help >/dev/null 2>&1 || { echo "FAIL: lark-cli drive not available"; exit 1; }
echo "  OK: drive"

echo "supper-lark-doc-ingest smoke: checking reference files..."
...
echo "supper-lark-doc-ingest smoke: verifying drive +search with date flag..."
...
echo "supper-lark-doc-ingest smoke: PASS"
