...
# polym-meeting-recorder — Live Meeting Recording

Use this skill when the user wants to start, monitor, summarize, or end a live meeting recording.

This is different from `polym-meeting-summary`:

| Skill | When | Source |
|---|---|---|
| `polym-meeting-recorder` | During a live meeting | Syncore mic + system audio recording |
| `polym-meeting-summary` | After meetings already ended | Lark VC + Lark Minutes history |
...
- `polym-sa-wiki` skill for Lark Wiki writes
...
1. Read `polym-sa-wiki` SKILL.md write workflow.
...
2. If missing, ask once whether to create one using `polym-local-wiki-init`.
3. If available, save:
...
| `polym-sa-wiki` write fails | Keep the Syncore finalized note and tell the user knowledge-base save needs retry |
