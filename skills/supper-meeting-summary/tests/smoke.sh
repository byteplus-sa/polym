#!/usr/bin/env bash
# Smoke test for supper-meeting-summary skill
set -euo pipefail

echo "supper-meeting-summary smoke: checking lark-cli..."
command -v lark-cli >/dev/null || { echo "FAIL: lark-cli not found"; exit 1; }

echo "supper-meeting-summary smoke: checking vc and minutes commands..."
...
echo "supper-meeting-summary smoke: checking reference files..."
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
[[ -f "$SKILL_DIR/references/meeting-fetch.md" ]] || { echo "FAIL: missing meeting-fetch.md"; exit 1; }

echo "supper-meeting-summary smoke: PASS"
