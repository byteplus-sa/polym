...
1. `cp -r skills/_template skills/polymath-<name>` — never start from a blank folder.
...
```yaml
name: polymath-meeting-summary        # kebab-case, globally unique, must start with polymath-
...
description_for_install: |
  Summarises a Lark meeting and writes a structured signal into polymath-sa-wiki.

depends_on:
  skills:                           # other skills in THIS repo
    - polymath-sa-wiki: ">=0.2.0 <1.0.0"
    - lark-minutes: "*"
...
Why: each skill has to install standalone. A user might install `polymath-meeting-summary`
without `polymath-sa-wiki`, and the CLI should detect missing deps and warn, not crash
