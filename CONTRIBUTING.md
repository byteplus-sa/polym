...
1. `cp -r skills/_template skills/polym-<name>` — never start from a blank folder.
...
```yaml
name: polym-meeting-summary        # kebab-case, globally unique, must start with polym-
...
description_for_install: |
  Summarises a Lark meeting and writes a structured signal into polym-sa-wiki.

depends_on:
  skills:                           # other skills in THIS repo
    - polym-sa-wiki: ">=0.2.0 <1.0.0"
    - lark-minutes: "*"
...
io_contract:                        # what data this skill reads / writes
  ...
  writes:
    - target: polym-sa-wiki/write_queue
...
Why: each skill has to install standalone. A user might install `polym-meeting-summary`
without `polym-sa-wiki`, and the CLI should detect missing deps and warn, not crash
